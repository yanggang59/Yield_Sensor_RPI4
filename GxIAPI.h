...
typedef enum GX_STATUS_LIST
{
    ...
    GX_STATUS_NEED_MORE_BUFFER          = -9,          ///< The user request buffer is insufficient: the user input buffersize during the read operation is less than the actual need
...
enum GX_IP_CONFIGURE_MODE_LIST
{
    ...
    GX_IP_CONFIGURE_LLA                 = 0x4,         ///< Enable the LLA mode to allocate the IP addresse
...
typedef enum GX_FEATURE_ID
{
    ...
    GX_STRING_DEVICE_SERIAL_NUMBER             = 4   | GX_FEATURE_STRING | GX_FEATURE_LEVEL_REMOTE_DEV,   ///< Device’s serial number.
    ...
    GX_INT_DEVICE_LINK_CURRENT_THROUGHPUT      = 11  | GX_FEATURE_INT    | GX_FEATURE_LEVEL_REMOTE_DEV,   ///< The bandwidth of current device acquistition
    ...
    GX_ENUM_REGION_SEND_MODE          = 1019 | GX_FEATURE_ENUM | GX_FEATURE_LEVEL_REMOTE_DEV,    ///< ROI output mode, see alse GX_REGION_SEND_MODE_ENTRY
    ...
    GX_FLOAT_TRIGGER_FILTER_RAISING   = 3011 | GX_FEATURE_FLOAT | GX_FEATURE_LEVEL_REMOTE_DEV,   ///< Raisingedge signal pulse width is smaller than this value is invalid.
    GX_FLOAT_TRIGGER_FILTER_FALLING   = 3012 | GX_FEATURE_FLOAT | GX_FEATURE_LEVEL_REMOTE_DEV,   ///< Fallingedge signal pulse width is smaller than this value is invalid.
    ...
    GX_INT_LINE_STATUS_ALL            = 4009 | GX_FEATURE_INT  | GX_FEATURE_LEVEL_REMOTE_DEV,   ///< Returns the current status of all available Line signals at time of polling in a single bitfield.
    ...
    GX_ENUM_DEAD_PIXEL_CORRECT        = 5010 | GX_FEATURE_ENUM | GX_FEATURE_LEVEL_REMOTE_DEV,   ///< The deadpixel correct function can eliminate dead pixels in the image, see also GX_DEAD_PIXEL_CORRECT_ENTRY
...
{
    GX_EXPOSURE_MODE_TIMED                         = 1,           ///< Control exposure time throungh exposure time register 
    GX_EXPOSURE_MODE_TRIGGERWIDTH                  = 2,           ///< Control exposure time throungh trigger signal width
...
//----------------------------------------------------------------------------------
/**
\brief     Inquire the current camera whether support a special feature. Usually the camera doesn’t support a
           feature means that:
...
\return    GX_STATUS_SUCCESS               The operation is successful and no error occurs.
           ...
           GX_STATUS_NEED_MORE_BUFFER      Insufficient buffer requested by the user: When reading,the user 
                                           input buffersize is smaller than the actual need.
           GX_STATUS_TIMEOUT               Acquire image timeout error.
           GX_STATUS_ERROR                 Unspecified internal errors that are not expected to occur.
           The errors that are not covered above please reference GX_STATUS_LIST. 
*/
...
\return    GX_STATUS_SUCCESS               The operation is successful and no error occurs.
           GX_STATUS_NOT_INIT_API          The GXInitLib initialization library is not called.
           GX_STATUS_INVALID_HANDLE        The handle that the user introduces is illegal.，或者关闭已经被关闭的设备
           The errors that are not covered above please reference GX_STATUS_LIST.  
*/
//----------------------------------------------------------------------------------
...
\param     [in]pszDevcieMacAddress The MAC address of the device.
