...
void ImageProcessing(Mat& thisFrame, Mat& mask, float volume_weight, float CSA,float& this_weight,float& sum_weight,float coeff)
{
	...
	*/
	...
	/*查看A区域是否有点
	*/
	for (int i = 148; i < 336; i++) //i代表行，也就是V
	{
		if (findPeakInA) break;
		for (int j = 887; j < 998; j++)//j代表列，也就是U
		{
	...
	*/
	...
	if (!isVoid)
	{
		for (int i = 730; i >= 572; i--)
		{
			if (findBottom) break;
			for (int j = 1115; j > 1099; j--)
			{
				if (thisFrame.at<uchar>(i, j)>0)
				{
					findBottom = true;
	...
	if (!isVoid && findBottom)
	{
		...
		if (distance > 16) distance = distance - 10.0;
		else distance = 0.0;
		
		//计算出产量，单位为g
        this_weight = CSA * distance / 10.0 * volume_weight/coeff;
